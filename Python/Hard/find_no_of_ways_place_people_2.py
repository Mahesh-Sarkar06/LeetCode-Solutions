...
class Solution:
    def numberOfPairs(self, points: List[List[int]]) -> int:
        ...
        count = 0
        # Sort the points firstly ascending order of x-coord
        # and if same x-coord then sort in descending order of y-coord
        points.sort(key = lambda x: (x[0], -x[1]))

        # Iterate through each coordinates
        for i, (x, y) in enumerate(points):
            # Defining a variable at -ve infinity
            my = -inf
            # For every next coordinate pairs
            for bx, by in points[i+1:]:
                # Check if the y-coord lie in between the prev and -ve infinity point
                if my < by <= y:
                    # Assign that coordinate to defined variable and increase the count
                    my = by
                    count += 1
